...
export type TagItemMode = 'readonly' | 'editable'

export interface TagItemLabels {
  title: string
  description: string
  name: string
  color: string
...
export function TagItem({ tag, mode = 'readonly', labels, onChange, className }: TagItemProps) {
  ...
  if (mode === 'readonly') {
    return (
      <Badge variant="outline" className={cn('font-normal text-foreground/90', className)} style={getPlanTagColorStyle(tag.color)}>
        {tag.name}
      </Badge>
    )
  }
  ...
  if (!labels) {
    throw new Error('TagItem requires labels in editable mode.')
  }
